...
bp = Blueprint('traffic_speed2', __name__)

@bp.route('/', methods=['GET'])
def get_traffic_speed2():
    try:
        cur = mysql.connection.cursor()
        cur.execute("SELECT traffic_speed2 FROM customers WHERE traffic_speed2 IS NOT NULL ORDER BY id DESC LIMIT 1")
        ...
        return jsonify({"traffic_speed2": traffic_speed}), 200
...
def add_traffic_speed2():
    data = request.json
    current_traffic_speed = data.get('traffic_speed2')
    try:
        cur = mysql.connection.cursor()
        cur.execute("INSERT INTO customers (traffic_speed2) VALUES (%s)", (current_traffic_speed,))
